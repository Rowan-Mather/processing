...
int randomPosX = int(random(width));
int randomPosY = int(random(height));
int m = millis();
int pesTime = 0;
...
void move() {
  if (food == true) {
    ...
    if (((fishX == randomPosX) || (fishX-1 == randomPosX) || (fishX+1 == randomPosX)) && ((fishY == randomPosY) || (fishY-1 == randomPosY) || (fishY+1 == randomPosY))) {
      randomPosX = int(random(width));
      randomPosY = int(random(height));
      pesTime = millis();
    }
    if (m-pesTime > 4000) {
      randomPosX = int(random(width));
